...
def get_all_tasks(creds=None):
    ...
    for course in courses:
        ...
        for cw in course_works:
            ...
            # Enlaces de Drive ultrarrápidos sin descargar PDFs pesados
            materials = cw.get('materials', [])
            attachment_links = []

            for m in materials:
                df = m.get('driveFile', {}).get('driveFile', {})
                if df and df.get('alternateLink'):
                    attachment_links.append(f"{df.get('title', 'Documento adjunto')} ({df.get('alternateLink')})")
...
def get_announcements_and_alerts(creds=None):
    ...
    try:
        ...
        cutoff_date = (datetime.datetime.utcnow() - datetime.timedelta(days=2)).isoformat() + "Z"
        ...
        if gmail_service:
            try:
                query = "newer_than:2d (clase OR suspende OR asistencia OR aviso OR cancela OR examen OR práctica)"
