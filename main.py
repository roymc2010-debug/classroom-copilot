...
def restore_and_refresh_credentials(creds_json: str):
    """Restaura y auto-refresca silenciosamente las credenciales si ya paso mas de 1 hora"""
    if not creds_json:
        return None
    try:
        data = json.loads(creds_json)
        creds = Credentials.from_authorized_user_info(data, SCOPES)
        # Si el token de 60 minutos ya vencio, pedir uno nuevo en segundo plano
...
# API Tasks con auto-refresco garantizado
@app.get("/api/tasks")
...
async def ask_ai(request: Request):
    ...
    return {"response": response_text}import os
import json
import uuid
import urllib.request
import urllib.parse
