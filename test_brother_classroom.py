...
# Permitir uso de bibliotecas de Google si están instaladas en el entorno virtual
try:
...
def get_real_session_token_and_creds():
    """
    Recupera el token de sesión real y credenciales almacenadas en sessions.json (localhost)
    para ejecutar pruebas directas contra la cuenta real de Google Classroom sin datos inventados.
    """
    ...
    if os.path.exists(sessions_file):
        try:
            ...
            for sid, sdata in sessions.items():
                if not sdata.get("is_demo") and sdata.get("creds"):
                    import main
                    creds = main.restore_and_refresh_credentials(sdata, session_id=sid)
                    if creds and creds.valid:
                        return sid, sdata.get("email"), creds
        except Exception as e:
            print(f"Aviso cargando credenciales reales de sessions.json: {e}")
...
class TestBrotherClassroomData(unittest.TestCase):
    ...
    def test_6_extract_actionable_consignas(self):
        """
        Prueba que el extractor de consignas detecte ejercicios reales
        e ignore encabezados institucionales y nombres de materias.
        Si hay un token de sesión local en localhost (sessions.json), prueba contra documentos reales de Classroom.
        """
        from services.classroom_service import extract_actionable_consignas, get_drive_service, extract_pdf_text_from_drive
        sid, email, creds = get_real_session_token_and_creds()
        if creds:
            drive_svc = get_drive_service(creds)
            # Documento real de Classroom: Teoria de sistemas II practica 1.pdf
            real_file_id = "11yEM1HLGKOZdTQ4-pYYqwUth54DV4xA8"
            real_text = extract_pdf_text_from_drive(drive_svc, real_file_id)
            self.assertTrue(len(real_text) > 50, f"No se pudo leer texto del documento real {real_file_id}")
            actions = extract_actionable_consignas(real_text, course_name="TEORIA DE SISTEMAS II 2026B  9am", task_title="Teoría de sistemas II practica 1")
            self.assertGreaterEqual(len(actions), 1)
            actions_text = " ".join(actions).lower()
            self.assertTrue(any(w in actions_text for w in ["laboratorio", "equipos", "visita", "inspección", "práctica", "r5"]),
                            f"Las acciones extraídas de la tarea real no contienen términos clave: {actions}")
            print(f"[OK] Test 6 superado (EN VIVO CON CLASSROOM REAL): Extractor obtuvo {len(actions)} consignas reales del documento '{real_file_id}' de {email}.")
        else:
            sample_text = (
            ...
            print("[OK] Test 6 superado (Mock fallback): Extractor obtiene consignas y ejercicios reales omitiendo ruido institucional.")

    def test_7_attachment_cache_persistence(self):
        """
        Prueba que las consignas y texto extraídos se almacenen y recuperen en caché de disco data/pdf_cache/{file_id}.json.
        Con token de sesión local, prueba la persistencia del archivo real de Classroom.
        """
        from services.classroom_service import get_cached_attachment_data, save_cached_attachment_data, get_drive_service, extract_pdf_text_from_drive, extract_actionable_consignas
        sid, email, creds = get_real_session_token_and_creds()
        if creds:
            drive_svc = get_drive_service(creds)
            real_file_id = "1fuG2ilcWFs2n5g9J2xV4gZbDb78yfqoQ" # Tarea 2: Ecuaciones características
            real_text = extract_pdf_text_from_drive(drive_svc, real_file_id)
            actions = extract_actionable_consignas(real_text, course_name="SEMINARIO DE PROBLEMAS DE MODELADO Y SIMULACIÓN DE SISTEMAS", task_title="Tarea 2: Ecuaciones características")
            test_data = {
                "file_id": real_file_id,
                "actions": actions,
                "text": real_text[:3000]
            }
            save_cached_attachment_data(real_file_id, test_data)
            loaded = get_cached_attachment_data(real_file_id)
            self.assertEqual(loaded.get("file_id"), real_file_id)
            self.assertIn("masa-resorte", loaded.get("text", "").lower())
            print(f"[OK] Test 7 superado (EN VIVO CON CLASSROOM REAL): Persistencia en caché verificada con archivo real de Drive '{real_file_id}' ({len(loaded.get('text', ''))} caracteres).")
        else:
            ...
            print("[OK] Test 7 superado (Mock fallback): Almacenamiento y recuperación en caché de disco de adjuntos es instantáneo y persistente.")
    ...
    def test_13_demo_mode_auth_and_data_serving(self):
        ...
        self.assertIn("agora_session=agora_demo_session", demo_resp.headers.get("set-cookie", ""))

        # Llamar a /api/tasks con la cookie de sesión demo
        client_with_cookie = TestClient(app)
        client_with_cookie.cookies.set("agora_session", "agora_demo_session")
    ...
    def test_14_teacher_task_pdfs_in_course_notes(self):
        ...
        Con token de sesión local, utiliza las tareas reales obtenidas de Google Classroom.
        """
        import services.notes_service as notes_service
        sid, email, creds = get_real_session_token_and_creds()
        if creds:
            from services.classroom_service import fetch_tasks
            real_tasks = fetch_tasks(creds)
            course = "TEORIA DE SISTEMAS II 2026B  9am"
            ...
            p1_doc = next((d for d in docs if "practica 1" in d.get("name", "").lower() or "teoria" in d.get("name", "").lower()), None)
            self.assertIsNotNone(p1_doc, f"No se encontró documento de la tarea real en {docs}")
            self.assertTrue(p1_doc.get("available"))
            self.assertIn(f"authuser={email}", p1_doc["preview_url"])
            print(f"[OK] Test 14 superado (EN VIVO CON CLASSROOM REAL): get_course_notes listó {len(docs)} documentos reales para '{course}'.")
        else:
            ...
            notes_service.delete_course_notes(course)
            print("[OK] Test 14 superado (Mock fallback): PDFs de las tareas del profesor se incorporan exitosamente a los documentos de la materia.")
    ...
    def test_15_api_notes_endpoint_serves_task_pdfs(self):
        ...
        Con token de sesión local, prueba con la cookie real de agora_session.
        """
        from fastapi.testclient import TestClient
        ...
        sid, email, creds = get_real_session_token_and_creds()
        if creds and sid:
            client = TestClient(app, cookies={"agora_session": sid})
            course_name = "TEORIA DE SISTEMAS II 2026B  9am"
            encoded_course = urllib.parse.quote(course_name)
            resp = client.get(f"/api/notes/{encoded_course}")
            ...
            self.assertIsNotNone(p1_doc, f"No se encontró documento de la tarea real en {docs}")
            self.assertTrue(p1_doc.get("available"))
            print(f"[OK] Test 15 superado (EN VIVO CON CLASSROOM REAL): Endpoint /api/notes/{course_name} sirvió {len(docs)} documentos reales de Classroom.")
        else:
            user_sessions["agora_demo_session"] = {
                "email": ALEX_EMAIL,
                "is_demo": True
            }
            client = TestClient(app, cookies={"agora_session": "agora_demo_session"})
            ...
            print("[OK] Test 15 superado (Mock fallback): Endpoint /api/notes sirve instantáneamente los PDFs de tareas del profesor.")
    ...
    def test_18_web_push_and_background_sync(self):
        ...
        test_email = "estudiante.demo@agora.edu.mx"
    ...
    def test_21_real_classroom_document_reading(self):
        """
        Prueba Integral en Vivo: Lectura de documentos y consignas de Google Classroom
        utilizando el token de sesión local de localhost (sessions.json) sin datos inventados.
        Verifica:
        1. Lectura y extracción directa desde Drive de los PDFs de tareas reales.
        2. Lectura y extracción de archivos Word .docx subidos por el profesor.
        3. Detección de consignas reales del profesor sin mocks.
        4. Interacción con el copiloto Ignis en /api/copilot/ask usando el contexto real.
        """
        from services.classroom_service import get_drive_service, extract_pdf_text_from_drive, extract_actionable_consignas
        from fastapi.testclient import TestClient
        from main import app

        sid, email, creds = get_real_session_token_and_creds()
        self.assertIsNotNone(creds, "No se encontró sesión con credenciales válidas en sessions.json")
        self.assertIsNotNone(sid, "Session ID no encontrado")

        drive_svc = get_drive_service(creds)
        self.assertIsNotNone(drive_svc, "No se pudo inicializar Google Drive Service con credenciales de sesión")

        # 1. Caso 1: Lectura de PDF real de 'Teoría de sistemas II practica 1'
        fid_p1 = "11yEM1HLGKOZdTQ4-pYYqwUth54DV4xA8"
        text_p1 = extract_pdf_text_from_drive(drive_svc, fid_p1)
        self.assertGreater(len(text_p1), 100, f"Texto insuficiente en {fid_p1}")
        self.assertIn("laboratorio de control 1", text_p1.lower())
        self.assertIn("r5", text_p1.lower())
        norm_p1 = " ".join(text_p1.lower().split())
        self.assertIn("aula r5", norm_p1)
        consignas_p1 = extract_actionable_consignas(text_p1, course_name="TEORIA DE SISTEMAS II 2026B  9am", task_title="Teoría de sistemas II practica 1")
        self.assertGreaterEqual(len(consignas_p1), 1)

        # 2. Caso 2: Lectura de PDF real de 'Tarea 2: Ecuaciones características...'
        fid_t2 = "1fuG2ilcWFs2n5g9J2xV4gZbDb78yfqoQ"
        text_t2 = extract_pdf_text_from_drive(drive_svc, fid_t2)
        self.assertGreater(len(text_t2), 100, f"Texto insuficiente en {fid_t2}")
        self.assertIn("masa-resorte", text_t2.lower())
        self.assertIn("matlab", text_t2.lower())
        consignas_t2 = extract_actionable_consignas(text_t2, course_name="SEMINARIO DE PROBLEMAS DE MODELADO Y SIMULACIÓN DE SISTEMAS", task_title="Tarea 2: Ecuaciones características")
        self.assertGreaterEqual(len(consignas_t2), 1)

        # 3. Caso 3: Lectura de documento Word .docx real
        fid_docx = "1-w16I8TbvyBIP6s7XkRYbERu37pKY2AP"
        text_docx = extract_pdf_text_from_drive(drive_svc, fid_docx)
        self.assertGreater(len(text_docx), 50, f"Texto insuficiente extraído del archivo Word {fid_docx}")
        self.assertIn("rubrica", text_docx.lower())

        # 4. Verificación en API: /api/copilot/ask con sesión real del alumno
        client = TestClient(app, cookies={"agora_session": sid})
        ask_resp = client.post("/api/copilot/ask", json={
            "provider": "openrouter",
            "mentor": "auto",
            "stream": False,
            "task_id": "866979309375",
            "file_id": fid_p1,
            "document_title": "Teoria de sistemas II practica 1.pdf",
            "task_context": {
                "course_name": "TEORIA DE SISTEMAS II 2026B  9am",
                "title": "Teoría de sistemas II practica 1"
            },
            "messages": [
                {"role": "user", "content": "¿Cuáles son los primeros pasos que debo seguir según este documento?"}
            ]
        })
        self.assertEqual(ask_resp.status_code, 200)
        resp_data = ask_resp.json()
        ai_reply = resp_data.get("response", "")
        self.assertGreater(len(ai_reply), 30)

        print(f"[OK] Test 21 superado (EN VIVO CON CLASSROOM REAL): Lectura y validación de documentos reales de Classroom superada exitosamente para el estudiante {email}.")
